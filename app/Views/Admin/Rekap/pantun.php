...
<head>
    ...
    <title>GLN GAREULIS - Admin Rekap Karya Tulis (pantun)</title>
...
<main role="main" class="flex-shrink-0">
    <div class="container">
        <h1 class="my-5">Rekap Data Karya Tulis (pantun)</h1>
        <a href="<?php echo route_to('rekap-karya');?>" class="btn btn-outline-info text-decoration-none mb-2">Naskah</a>
        <a href="<?php echo route_to('rekap-puisi');?>" class="btn btn-outline-secondary text-decoration-none mb-2">Puisi</a>
        <table id="rekapPantunTable" class="table table-striped table-bordered" style="width:100%">
            <thead>
                <tr>
                    <th>No</th>
                    <th>NIK Puisi</th>
...
<script type="text/javascript">
    ...
    const api_uri = "<?php echo route_to('datatable-pantun'); ?>";

    $(function(){

        let biodata_table = new $('#rekapPantunTable').DataTable({
            "ordering" : false,
            // "filtering" : false,
            "ajax" : {
                "url" : api_uri,
            ...
            "columns" : [
                ...
                { "data" : 'pantun_naskah' },
                { "data" : 'created_at'},
                { "data" : 'link_pantun', 'className' : 'hideThis'}
            ],
            dom: 'Bfrtip',
            buttons: [
                'csv', 'excel', 'pdf', 'print'
        ...
        $('.buttons-print').on('click', function(e){
            $('#rekapPantunTable').removeClass('hideThis');
            setTimeout(2000);
            $('#rekapPantunTable').addClass('hideThis');
        });
